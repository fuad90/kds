...
class PeopleController extends Controller
{
    ...
    public function actionView($id)
    {
        ...
        $model_wife = new ActiveDataProvider([
            'query' => Wife::find()->where(['people_id'=>$id]),
        ]);
        $model_tanggungan = new ActiveDataProvider([
            'query' => Tanggungan::find()->where(['people_id'=>$id]),
        ]);
        $model_answer = new ActiveDataProvider([
            'query' => Answer::find()->where(['people_id'=>$id]),
            'pagination' => ['pageSize'=>52],
        ]);
        $model_tanggunganoku = new ActiveDataProvider([
            'query' => TanggunganOku::find()->where(['people_id'=>$id]),
            'pagination' => ['pageSize'=>52],
        ]);
